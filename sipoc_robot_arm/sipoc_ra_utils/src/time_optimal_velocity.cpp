...
namespace sipoc_ra_utils
{
    ...
    auto TimeOptimalVelocity::solve(VelocityNode seed, const SplineWrapper8Dim &path, const std::vector<double> &grid) const
    ...
    auto TimeOptimalVelocity::solve_step1_geometry(const SplineWrapper8Dim &path, const std::vector<double> &grid) const
        -> std::vector<VelocityNode>
    {
    ...
    auto TimeOptimalVelocity::solve_step2_velocity(const SplineWrapper8Dim &path,
                                                   const std::vector<double> &grid,
    ...
    double TimeOptimalVelocity::computeAllowedVelocity(const SplineWrapper8Dim &path, double coordinate) const
    {
    ...
    double TimeOptimalVelocity::computeAllowedAcceleration(const SplineWrapper8Dim &path,
                                                           double coordinate,
    ...
    double TimeOptimalVelocity::computeAllowedDeceleration(const SplineWrapper8Dim &path,
                                                           double coordinate,
                                                           double velocity) const
    {
    ...
    double TimeOptimalVelocity::simulateVelocity(const SplineWrapper8Dim &path,
    ...
    double TimeOptimalVelocity::simulateTime(const SplineWrapper8Dim &path,
                                             double current_coordinate,
                                             double next_coordinate,
                                             double current_velocity,
    ...
    double TimeOptimalVelocity::simulateDistance(const SplineWrapper8Dim &path,
                                                 double current_coordinate,
                                                 double next_coordinate,
                                                 double current_distance) const
    {
        double tangent_norm_current = path.p(current_coordinate).norm();
        double tangent_norm_next = path.p(next_coordinate).norm();
        double dth = next_coordinate - current_coordinate;
        return current_distance + 0.5 * dth * (tangent_norm_current + tangent_norm_next);
    }

    double TimeOptimalVelocity::singleStep(const SplineWrapper8Dim &path,
                                           double current_coordinate,
                                           double next_coordinate,
                                           double current_velocity,
