...
namespace sipoc_mr_solver
{
    ...
    void DilatedPolygonNominalController::checkOCPSameDimsAsExported()
    {
    ...
    void DilatedPolygonNominalController::createAcadosSolver()
    {
    ...
    void DilatedPolygonNominalController::configureLowerLevelSolver()
    {
        ptr_sd_map_ = std::make_unique<sipoc_mr_utils::SignedDistanceMap>(params_.sdf, params_.robot);
        ptr_ll_solver_ = std::make_unique<sipoc_mr_utils::DistanceMinimizerPointToPolygon>(params_.robot);
    }

    bool DilatedPolygonNominalController::solveOCPSubproblem(int idx_iter)
    {
    ...
    int DilatedPolygonNominalController::getObsWithMaximumGVal(int idx_hrzn)
    {
    ...
    bool DilatedPolygonNominalController::solveEndpointGammaConstrainedSubproblem()
    {
    ...
    void DilatedPolygonNominalController::findClosestObsGivenGamma(const Eigen::Vector2d &gamma_val, unsigned int idx_constr, unsigned int idx_hrzn)
    {
        manif::SE2<double> pose_at_tk(solPxAtTk(idx_hrzn), solPyAtTk(idx_hrzn), solYawAtTk(idx_hrzn));
    ...
    void DilatedPolygonNominalController::updateGammaAndSepPlane(int idx_hrzn, int num_imposed_constr)
    {
        ...
        for (int idx_obs = 0; idx_obs < num_imposed_constr; ++idx_obs)
        {
            ...
            obs_inside_polygon = ptr_ll_solver_->ifInsidePolygon(solYawAtTk(idx_hrzn), lower_level_param_.p_center_ellipsoid);
