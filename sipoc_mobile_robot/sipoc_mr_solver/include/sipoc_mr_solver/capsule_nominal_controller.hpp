...
#ifndef SIPOC_MR_CONTROLLER__CAPSULE_NOMINAL_CONTROLLER_HPP_
#define SIPOC_MR_CONTROLLER__CAPSULE_NOMINAL_CONTROLLER_HPP_

#include "sipoc_mr_solver/base_controller.hpp"
#include "sipoc_mr_utils/distance_minimizer_point_to_line.hpp"
#include "acados_solver_diff_drive_robot.h"

namespace sipoc_mr_solver
{
    class CapsuleNominalController : public BaseController
    {
    public:
        CapsuleNominalController() : BaseController()
        {
            flag_constr_tightening_ = 0.0;
            checkOCPSameDimsAsExported();
        }

        CapsuleNominalController(int n_hrzn, double delta_t) : BaseController(n_hrzn, delta_t)
        {
            flag_constr_tightening_ = 0.0;
            checkOCPSameDimsAsExported();
        }

        CapsuleNominalController(sipoc_mr_utils::OCPParameters &param) : BaseController(param)
        {
            flag_constr_tightening_ = 0.0;
            checkOCPSameDimsAsExported();
        }
        ~CapsuleNominalController()
        {
    ...
    protected:
        ...
        std::unique_ptr<sipoc_mr_utils::DistanceMinimizerPointToLine<double>> ptr_ll_solver_;
...
#endif // SIPOC_MR_CONTROLLER__CAPSULE_NOMINAL_CONTROLLER_HPP_
