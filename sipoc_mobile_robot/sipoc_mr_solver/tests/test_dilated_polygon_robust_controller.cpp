...
int main()
{
    ...
    params.disturbance.robust_scale = 1.0;

    sipoc_mr_solver::DilatedPolygonRobustController solver(params);
    ...
    std::ofstream csv_cov_file;
    csv_cov_file.open("csv_cov.csv");
    ...
    {
        solver.grads_valid4prop = false;
        auto t1 = high_resolution_clock::now();
        solver.updateInitialState(current_pose, current_ref_vel);
        ...
        csv_cov_file << "# This is idx_test=" << idx_test << ".\n";
        Eigen::Matrix<double, 9, 9> unc_matrix;
        for (unsigned int idx_hrzn = 0; idx_hrzn <= static_cast<unsigned int>(n_hrzn); ++idx_hrzn)
        {
            solver.uncMatrixAtTk(idx_hrzn, unc_matrix);
            for (unsigned int ii = 0; ii < params.ocp_dims.nx; ++ii)
            {
                for (unsigned int jj = 0; jj < params.ocp_dims.nx; ++jj)
                {
                    csv_cov_file << unc_matrix.coeff(ii, jj);
                    if ((ii < (params.ocp_dims.nx - 1) || jj < (params.ocp_dims.nx - 1)))
                    {
                        csv_cov_file << ",";
                    }
                }
