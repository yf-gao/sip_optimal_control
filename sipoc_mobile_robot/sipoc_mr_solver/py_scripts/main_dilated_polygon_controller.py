...
utils_lib_path = os.path.join(utils_prefix_path, 'lib')
sys.path.append(utils_lib_path)
solver_prefix_path = get_package_prefix("sipoc_mr_solver")
solver_lib_path = os.path.join(solver_prefix_path, 'lib')
sys.path.append(solver_lib_path)
...
from sipoc_plot_utils.plot_utils import plot_dilated_polygon_on_axis, compute_dilated_polygon_vertices

from py_sipoc_mr_utils import py_vel_profile_optimizer, py_sd_map
from py_sipoc_mr_solver import py_dilated_polygon_nominal_controller, py_dilated_polygon_robust_controller, py_solver_status
...
def test(world_type:str, flag_robustOCP: bool = False):
    world_dist = {
        "L_corridor":
        {
    ...
    robot_cfg.robot_polygon_vertices_flattened = np.array([-0.18, -0.11, 0.45, -0.11, 0.45,  0.11, -0.18,  0.11, -0.33,  0.])
    ...
    dilated_polygon_vertices = compute_dilated_polygon_vertices(np.reshape(robot_cfg.robot_polygon_vertices_flattened, (-1, 2)), robot_cfg.robot_polygon_radius)
    ...
    def animate_solution(result: SIPResult, num_hrzn: int):
        ...
        for idx_hrzn in range(num_hrzn + 1):
            ...
            rotation_mat = np.array([[np.cos(result.x_sol[idx_hrzn, 2]), -np.sin(result.x_sol[idx_hrzn, 2])], [np.sin(result.x_sol[idx_hrzn, 2]), np.cos(result.x_sol[idx_hrzn, 2])]])
            num_considered_obs = 0
            while (num_considered_obs < result.num_constr[idx_hrzn]):
                if np.allclose(result.obs_sol[idx_hrzn, 2*num_considered_obs:], 0.) and np.allclose(result.plane_sol[idx_hrzn, 2*num_considered_obs:], 0.) and np.allclose(result.gamma_sol[idx_hrzn, 2*num_considered_obs:], 0.):
                    break
            ...
            for ii in range(num_considered_obs):
                ...
                ax.scatter(result.x_sol[idx_hrzn, 0]+np.dot(rotation_mat[0, :], result.gamma_sol[idx_hrzn, 2*ii:2*ii+2]), result.x_sol[idx_hrzn, 1]+np.dot(rotation_mat[1, :], result.gamma_sol[idx_hrzn, 2*ii:2*ii+2]), color=color_cycle_list[ii%len(color_cycle_list)], marker='v', s=80)
            plot_dilated_polygon_on_axis(ax, result.x_sol[[idx_hrzn], :], robot_cfg.robot_polygon_vertices_flattened.reshape((-1, 2)), dilated_polygon_vertices, color='tab:blue', plot_polygon=True)
            ...
            plt.draw()
            plt.pause(0.6)
            # plt.savefig(f"fig_dilated_polygon_{'robust' if flag_robustOCP else 'nominal'}_{world_type}_{idx_hrzn:03d}.png", dpi=200)
        plt.close(fig)

    ws_strategy = WarmStartStrategy.COLD_START
    if flag_robustOCP:
        foo = py_dilated_polygon_robust_controller.DilatedPolygonRobustController(solver_cfg.n_hrzn, solver_cfg.delta_t, polygon_dilation_radius=robot_cfg.robot_polygon_radius)
    else:
        foo = py_dilated_polygon_nominal_controller.DilatedPolygonNominalController(solver_cfg.n_hrzn, solver_cfg.delta_t, polygon_dilation_radius=robot_cfg.robot_polygon_radius)
    def solve_cpp():
        yref = np.hstack((ref_traj[:-1, :], np.zeros((solver_cfg.n_hrzn, 2))))
        yref_e = ref_traj[-1, :].copy()
        foo.setPolygonVertices(robot_cfg.robot_polygon_vertices_flattened)
        foo.setupOcpSolver()
    ...
    def get_cpp_solution():
        ...
        assert gamma_sol_cpp.shape[1] == 2*solver_cfg.constr_max_num, f"gamma_sol_cpp shape={gamma_sol_cpp.shape}, solver_cfg.constr_max_num={solver_cfg.constr_max_num}"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Test Dilated Polygon Controller")
    parser.add_argument("--robustify", action="store_true", help="Use robust OCP")
    parser.add_argument("--no_robustify", dest='robustify', action='store_false')
    parser.set_defaults(robustify=False)
    ...
    if args.robustify:
        print("Dilated-Polygon Shape, Robust Controller")
    else:
        print("Dilated-Polygon Shape, Nominal Controller")
    test(world_type, flag_robustOCP=args.robustify)
