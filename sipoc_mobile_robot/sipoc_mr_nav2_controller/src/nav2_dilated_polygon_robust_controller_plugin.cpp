...
#include "sipoc_mr_nav2_controller/nav2_dilated_polygon_robust_controller_plugin.hpp"
...
namespace sipoc_mr_nav2_controller
{
    ...
    void RobustDilatedPolygonController::configure(
        ...
        std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
    {
        ...
        ptr_controller_ = std::make_shared<sipoc_mr_solver::DilatedPolygonRobustController>(nav2_plugin_params_.ocp);
        ...
        P = Eigen::Matrix<double, 4, 4>::Identity() * 1e-6;
        ...
        pub_ustar_values_ = node->create_publisher<geometry_msgs::msg::AccelStamped>("ustar_t0", 1);

        // create subscription
        if (nav2_plugin_params_.if_subscribe_laser_scan)
        {
            sub_laser_scan_ = node->create_subscription<sensor_msgs::msg::LaserScan>("scan", 1, std::bind(&RobustDilatedPolygonController::FilteredLaserCallback, this, std::placeholders::_1));
            sub_occ_map_ = nullptr;
        }
        else
        {
            sub_occ_map_ = node->create_subscription<nav_msgs::msg::OccupancyGrid>("global_costmap", 1, std::bind(&RobustDilatedPolygonController::OccupancyMapCallback, this, std::placeholders::_1));
            sub_laser_scan_ = nullptr;
        }
    }

    void RobustDilatedPolygonController::declareAndGetParams()
    {
        auto node = node_.lock();
        ...
        declare_parameter_if_not_declared(
            node, plugin_name_ + ".robot.vertices_flattened", rclcpp::ParameterValue(nav2_plugin_params_.ocp.robot.vertices_flattened));
        declare_parameter_if_not_declared(
            node, plugin_name_ + ".robot.robot_polygon_dilation_radius", rclcpp::ParameterValue(nav2_plugin_params_.ocp.robot.robot_polygon_dilation_radius));
        node->get_parameter(plugin_name_ + ".robot.num_gamma_grids", nav2_plugin_params_.ocp.robot.num_gamma_grids);
        node->get_parameter(plugin_name_ + ".robot.vertices_flattened", nav2_plugin_params_.ocp.robot.vertices_flattened);
        node->get_parameter(plugin_name_ + ".robot.robot_polygon_dilation_radius", nav2_plugin_params_.ocp.robot.robot_polygon_dilation_radius);

        // disturbance
        declare_parameter_if_not_declared(
            node, plugin_name_ + ".disturbance.robust_scale", rclcpp::ParameterValue(nav2_plugin_params_.ocp.disturbance.robust_scale));
        node->get_parameter(plugin_name_ + ".disturbance.robust_scale", nav2_plugin_params_.ocp.disturbance.robust_scale);
        ...
        declare_parameter_if_not_declared(
            node, plugin_name_ + ".visualization.publish_ustar_values", rclcpp::ParameterValue(nav2_plugin_params_.visualization.publish_ustar_values));
        ...
        node->get_parameter(plugin_name_ + ".visualization.publish_ustar_values", nav2_plugin_params_.visualization.publish_ustar_values);
    ...
    void RobustDilatedPolygonController::activate()
    {
        RCLCPP_INFO(logger_, "Activating controller: sipoc_mr_nav2_controller::RobustDilatedPolygonController");
    ...
    void RobustDilatedPolygonController::deactivate()
    {
        RCLCPP_INFO(logger_, "Dectivating controller: sipoc_mr_nav2_controller::RobustDilatedPolygonController");
        ...
        pub_ustar_values_->on_deactivate();
    }

    void RobustDilatedPolygonController::cleanup()
    {
        RCLCPP_INFO(logger_, "Cleaning up controller: sipoc_mr_nav2_controller::RobustDilatedPolygonController");
        pub_ref_path_.reset();
        pub_ref_traj_.reset();
        ...
        pub_ustar_values_.reset();
        free(yref_);
        free(yref_e_);
    }

    void RobustDilatedPolygonController::setSpeedLimit(const double &speed_limit, const bool &percentage)
    {
    ...
    void RobustDilatedPolygonController::setPlan(const nav_msgs::msg::Path &path)
    {
        global_plan_ = path;
    }

    geometry_msgs::msg::TwistStamped RobustDilatedPolygonController::computeVelocityCommands(
        ...
        const geometry_msgs::msg::Twist &velocity, nav2_core::GoalChecker *)
    {
        ...
        Eigen::Matrix<double, 4, 4> Sigma0; // Dimension of the sys-id
        ptr_state_predictor_filter_->predictOCPInitialUncertainty(Sigma0);
        ptr_controller_->updateUncMatricesT0(Sigma0);
        ...
        if (!ptr_controller_->solver_initialized)
        {
            ...
            ptr_controller_->grads_valid4prop = false;
        }
    ...
    void RobustDilatedPolygonController::OccupancyMapCallback(nav_msgs::msg::OccupancyGrid msg)
    {
        if (!occ_map_updated_)
        {
    ...
    void RobustDilatedPolygonController::FilteredLaserCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
    {
        // MUTEX NEEDED?!?
    ...
    bool RobustDilatedPolygonController::buildReferencePath(const nav_msgs::msg::Path &global_plan,
    ...
    void RobustDilatedPolygonController::resetFilterBuffers()
    {
        cb_acc_hist_.clear();
    ...
    void RobustDilatedPolygonController::computeReferenceTraj(const manif::SE2<double> &current_pose,
    ...
    void RobustDilatedPolygonController::publishReferencePath(const rclcpp::Time &t)
    {
    ...
    void RobustDilatedPolygonController::publishReferenceTrajectory(const rclcpp::Time &t)
    {
    ...
    void RobustDilatedPolygonController::publishLocalPlan(const rclcpp::Time &t)
    {
    ...
    void RobustDilatedPolygonController::publishTimings(const rclcpp::Time &t)
    {
        if (nav2_plugin_params_.visualization.publish_timings)
        {
            ...
            msg.prop_disturb = ptr_controller_->prop_disturb_elapsed_time() * 1e-3;
    ...
    void RobustDilatedPolygonController::publishObstacle(const rclcpp::Time &t)
    {
    ...
    void RobustDilatedPolygonController::publishVelPredictOCP(const rclcpp::Time &t, const manif::SE2Tangent<double> &robot_twist)
    {
    ...
    void RobustDilatedPolygonController::publishUStar(const rclcpp::Time &t, const Eigen::Vector<double, 2> &u0_star)
    {
        if (nav2_plugin_params_.visualization.publish_ustar_values)
        {
            geometry_msgs::msg::AccelStamped msg;
            msg.header.stamp = t;
            msg.header.frame_id = "base_link";

            msg.accel.linear.x = u0_star.coeff(0);
            msg.accel.linear.y = 0.;
            msg.accel.angular.z = u0_star.coeff(1);

            pub_ustar_values_->publish(msg);
        }
    }
}

// Register this controller as a nav2_core plugin
PLUGINLIB_EXPORT_CLASS(sipoc_mr_nav2_controller::RobustDilatedPolygonController, nav2_core::Controller)
