...
namespace sipoc_mr_utils
{
    ...
    SplineFitter::SplineFitter(SplineWrapper spline, int32_t num_data) : spline_(std::move(spline))
    {
        initialize(num_data);
    }

    SplineFitter::SplineFitter(SplineWrapper spline, const DataGrid &data_grid) : spline_(std::move(spline))
    {
    ...
    void SplineFitter::init(uint32_t N, uint32_t degree, uint32_t nknots)
    {
        ...
        typename SplineWrapper::KnotVector knots(nknots, 0.0);
        for (uint32_t i = 0; i < knots.size(); ++i)
        {
            knots[i] = i / static_cast<double>(knots.size() - 1);
        }

        SplineWrapper spline(degree, SplineWrapper::ControlVector(nknots + degree - 1, SplineWrapper::Control::Zero()));
        spline.setKnotPoints(knots);
        this->setBSpline(std::move(spline), N);
    }

    void SplineFitter::setBSpline(SplineWrapper bspline, int32_t num_data)
    {
        spline_ = std::move(bspline);
        initialize(num_data);
    }

    void SplineFitter::setBSpline(SplineWrapper bspline, const DataGrid &data_grid)
    {
        spline_ = std::move(bspline);
        initialize(data_grid);
    }
    ...
    auto SplineFitter::fit(const DataContainer &data) const -> typename SplineWrapper::Controls
    {
        if (!spline_)
        {
    ...
    auto SplineFitter::bspline() const -> const SplineWrapper &
    {
        if (!spline_)
        {
            throw std::runtime_error("Spline not initialized.");
    ...
    auto SplineFitter::knots() const -> typename SplineWrapper::Knots
    {
    ...
    void SplineFitter::initialize(const DataGrid &data_grid)
    {
        ...
        {
            typename SplineWrapper::Regressor regr = spline_->regressor(data_grid[i]);
    ...
    auto SplineFitter::parseInput(const DataContainer &data) const
        -> Eigen::Matrix<double, Eigen::Dynamic, SplineWrapper::ny>
    {
        // Number of data points has been asserted already and must be greater than 1
        Eigen::Matrix<double, Eigen::Dynamic, SplineWrapper::ny> input(data.size(), data.front().rows());

        for (uint32_t i = 0; i < data.size(); ++i)
        {
            input.template block<1, SplineWrapper::ny>(i, 0) = data[i].transpose();
        }

        return input;
    }

    auto SplineFitter::parseOutput(const Eigen::Matrix<double, Eigen::Dynamic, SplineWrapper::ny> &c) const ->
        typename SplineWrapper::Controls
    {
        typename SplineWrapper::Controls output;
        const int32_t ny = c.cols();
        ...
        {
            output[i] = c.template block<1, SplineWrapper::ny>(i, 0).transpose();
        }
