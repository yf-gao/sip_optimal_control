...
namespace sipoc_mr_utils
{
    ...
    void SplineWrapper::configure(uint32_t degree, const ControlVector &controls)
    {
    ...
    void SplineWrapper::configure(const KnotVector &knots, const ControlVector &controls)
    {
    ...
    auto SplineWrapper::getDegree() const -> uint32_t
    {
        if (!isInitialized())
        {
    ...
    auto SplineWrapper::getNumControlPoints() const -> uint32_t
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
    ...
    auto SplineWrapper::getNumKnotPoints() const -> uint32_t
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
    ...
    void SplineWrapper::setKnotPoints(const KnotVector &knots)
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
    ...
    void SplineWrapper::setControlPoints(const ControlVector &controls)
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
        ...
        {
            gsl_matrix_set(interface_.controls.get(), 0, i, controls[i].x());
            gsl_matrix_set(interface_.controls.get(), 1, i, controls[i].y());
        }
    }

    auto SplineWrapper::getKnotPoints() const -> KnotVector
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
    ...
    auto SplineWrapper::getControlPoints() const -> ControlVector
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
        ...
        for (uint32_t i = 0; i < ncontrols; ++i)
        {
            ret[i].x() = gsl_matrix_get(interface_.controls.get(), 0, i);
            ret[i].y() = gsl_matrix_get(interface_.controls.get(), 1, i);
        }

        return ret;
    }

    auto SplineWrapper::eval(double x) const -> Output
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
        ...
        gsl_matrix_const_view controls_view =
            gsl_matrix_const_submatrix(interface_.controls.get(), 0, istart, 2, iend - istart + 1);

        Output result = Output::Zero();
        gsl_vector_view result_view = gsl_vector_view_array(result.data(), 2);
    ...
    auto SplineWrapper::p(double x) const -> OutputTangent
    {
        Output y;
        return p(x, y);
    }

    auto SplineWrapper::p(double x, Output &y) const -> OutputTangent
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
        ...
        gsl_vector_view y_view = gsl_vector_view_array(y.data(), 2);
        gsl_blas_dgemv(CblasNoTrans, 1, &controls_view.matrix, &B_view.vector, 0, &y_view.vector);

        return result;
    }

    auto SplineWrapper::pp(double x) const -> OutputTangent
    {
        Output y;
        OutputTangent p;
        return pp(x, y, p);
    ...
    auto SplineWrapper::pp(double x, Output &y, OutputTangent &p) const -> OutputTangent
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
        ...
        gsl_vector_view p_view = gsl_vector_view_array(p.data(), 2);
        gsl_blas_dgemv(CblasNoTrans, 1, &controls_view.matrix, &dB_view.vector, 0, &p_view.vector);

        return result;
    }

    auto SplineWrapper::curvature(double x) const -> double
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
    ...
    auto SplineWrapper::curvature_signed(double x) const -> double
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
        ...
        // det(g', g'') / ||g'||^3
        double n2px = px.squaredNorm();
        double npx = std::sqrt(n2px);
        return (px.x() * ppx.y() - px.y() * ppx.x()) / (n2px * npx);
    }

    auto SplineWrapper::regressor(double x) const -> Regressor
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
    ...
    auto SplineWrapper::supportLowerLimit() const -> double
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
    ...
    auto SplineWrapper::supportUpperLimit() const -> double
    {
        if (!isInitialized())
        {
            throw std::runtime_error("Spline not initialized.");
    ...
    inline auto SplineWrapper::isInitialized() const -> bool
    {
        return interface_.workspace ? true : false;
    }

    inline uint32_t SplineWrapper::computeDegree(uint32_t nbreakpts, uint32_t ncontrols)
    {
        return ncontrols - nbreakpts + 1;
    }

    inline uint32_t SplineWrapper::computeNumBreakPoints(uint32_t ncontrols, uint32_t degree)
    {
        return ncontrols - degree + 1;
    }

    void SplineWrapper::allocateInterface(Interface &interface, uint32_t degree, uint32_t nbreakpts, uint32_t ncontrols)
    {
        ...
        interface.controls =
            std::unique_ptr<gsl_matrix, void (*)(gsl_matrix *)>(gsl_matrix_alloc(2, ncontrols), gsl_matrix_free);
    }
