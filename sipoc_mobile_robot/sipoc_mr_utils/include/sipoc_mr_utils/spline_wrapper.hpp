...
#ifndef SIPOC_MR_UTILS__SPLINE_WRAPPER_HPP_
#define SIPOC_MR_UTILS__SPLINE_WRAPPER_HPP_
...
namespace sipoc_mr_utils
{
    ...
    public:
        ...
        using OutputTangent = Eigen::Matrix<double, 2, 1>;
        ...
        constexpr static int32_t ny = 2;

        SplineWrapper() = default;
        template <typename... Args, typename = std::enable_if_t<(sizeof...(Args) > 1)>>
        SplineWrapper(Args &&...args)
        {
            configure(std::forward<Args>(args)...);
        }

        virtual ~SplineWrapper() = default;

        SplineWrapper(const SplineWrapper &) = delete;
        SplineWrapper &operator=(const SplineWrapper &) = delete;
        SplineWrapper(SplineWrapper &&) = default;
        SplineWrapper &operator=(SplineWrapper &&) = default;
        ...
        auto curvature_signed(double x) const -> double;
...
#endif // SIPOC_MR_UTILS__SPLINE_WRAPPER_HPP_
