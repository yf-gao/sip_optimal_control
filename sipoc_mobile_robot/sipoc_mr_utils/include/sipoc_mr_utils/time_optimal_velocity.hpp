...
#ifndef SIPOC_MR_UTILS__TIME_OPTIMAL_VELOCITY_HPP_
#define SIPOC_MR_UTILS__TIME_OPTIMAL_VELOCITY_HPP_
...
namespace sipoc_mr_utils
{
    ...
    public:
         ...
         */
        using VelBoundFun = Bound(const SplineWrapper & /*path*/, double /*th*/);
        /**
         * Args: path, path coordinate [1], tangential velocity [m/s]
         * Ret: tangential velocity/acceleration bound [m/s^2]
         */
        using AccBoundFun = Bound(const SplineWrapper & /*path*/, double /*th*/, double /*sd*/);
        ...
        auto solve(VelocityNode seed, const SplineWrapper &path, const std::vector<double> &grid) const
            -> std::vector<VelocityNode>;
        auto solve_step1_geometry(const SplineWrapper &path, const std::vector<double> &grid) const
            -> std::vector<VelocityNode>;
        auto solve_step2_velocity(const SplineWrapper &path,
                                  const std::vector<double> &grid,
    ...
    private:
        ...
        ///
        double computeAllowedVelocity(const SplineWrapper &path, double coordinate) const;
        double computeAllowedAcceleration(const SplineWrapper &path, double coordinate, double velocity) const;
        double computeAllowedDeceleration(const SplineWrapper &path, double coordinate, double velocity) const;
        template <bool CheckZeroCrossing = false>
        double simulateVelocity(const SplineWrapper &path,
                                double current_coordinate,
                                double next_coordinate,
                                double current_velocity,
                                double acceleration) const;
        double simulateTime(const SplineWrapper &path,
                            double current_coordinate,
                            double next_coordinate,
                            double current_velocity,
                            double next_velocity,
                            double current_time) const;
        double simulateDistance(const SplineWrapper &path,
                                double current_coordinate,
                                double next_coordinate,
                                double current_distance) const;
        double singleStep(const SplineWrapper &path,
                          double current_coordinate,
                          double next_coordinate,
                          double current_velocity,
...
#endif // SIPOC_MR_UTILS__TIME_OPTIMAL_VELOCITY_HPP_
